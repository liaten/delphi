...
var
//чтобы использовать в другой форме переменную, создаем ее глобальной, и затем обращаемся к ней, как к свойству формы
...
begin
  ...
  begin
    ...
    if not loginquery.Recordset.EOF then
      begin
        ...
        chooseuser.Open;
        usertype:=chooseuser.FieldByName('pos').AsInteger;  //тип юзера, число, например, 5 - администратор
        idstaff:=chooseuser.FieldByName('idstaff').AsInteger;  //уникальный id пользователя в таблице стаф
        ...
        if usertype=5 then
        begin
           mainform.useridlabel.Caption:='Ваш ID пользователя: '+inttostr(usertype)+'. ';
        ...
        //mainform.useridlabel.Caption:=mainform.useridlabel.Caption+'Ваша должность: '+username+'.';
        usertypequery.Close;
        usertypequery.SQL.Clear;
        ...
        mainform.orglabel.Caption:='Ваша организация: '+orgname+'.';
        usertypequery.Close;
        usertypequery.SQL.Clear;
        ...
        mainform.label_fio.Caption:='Вас зовут: '+name+' '+surname+'.';
        for i := 0 to loginform.controlcount-1 do
        begin
          if loginform.controls[i] is tedit then
          begin
  ...
  begin
    if (loginedit.text='') and (passedit.text='') then
    begin
      Application.MessageBox('Заполните поле логина и поле пароля','Ошибка входа',MB_OK OR MB_ICONWARNING);
    end
    else if (loginedit.text='') then
    begin
      Application.MessageBox('Заполните поле логина','Ошибка входа',MB_OK OR MB_ICONWARNING);
    end
    else
    begin
      Application.MessageBox('Заполните поле пароля','Ошибка входа',MB_OK OR MB_ICONWARNING);
    end;
  end;
